...
String form =
  ...
  "<form action='msg'><p>Message <input type='text' name='msg' size=100 > <input type='submit' value='Submit'></form>"
  "<form action='config&restart'><p>Wifi SSID:  <input type='text' name='ssid' size=5> Pasword: <input type='text' name='pasword' size=5 autofocus > <input type='submit' value='Enregistre et redemarre'> </form>"
  ...
  "<a href=\"http://www.smart-diplay.kmt.orange.com\" target=\"_blank\">Lien page web</a>" 
  ;
...
{
  {"Temp", "Hygro","PsAt","Lum","Bruit","Alt","tempCaf","Message","Heure","Date","IP"},
...
{
  server.send(200, "text/html", form);    // Send a acquittal msg
  String ssid = server.arg("ssid");
  Serial.println("Ssid: ");
  Serial.println(ssid);
  String pasword = server.arg("pasword");
  Serial.println("Pasword: ");
  Serial.println(pasword);

  const int  ssidlength = ssid.length();
  const int  paswordlength = pasword.length();
  const int totalconfigLength = ssidlength  + paswordlength + 5;

String stringConfig =  "d/" + ssid +"/" +pasword + "/" ;
...
void handle_Configintensity()
{
  ...
  char buffintensity[] = " ";
  intensityString.toCharArray(buffintensity,intensityLength+1);
  ...
  if(intensity < 16)
  {
    ...
    for (int i = 100 ; i < 2 + 100 ; i ++)
    {
      EEPROM.write(i,buffintensity[i-100]);
    }
...
void handle_configSpeedDisplay()
{
  ...
  char buffspeedScroling[] = " ";
  speedScrolingString.toCharArray(buffspeedScroling,speedScrolingLength+1);
  ...
  if(speedScroling < 1000 & speedScroling > 10)
  {
    ...
    for (int i = 110 ; i < 3 + 110 ; i ++)
    {
      EEPROM.write(i,buffspeedScroling[i-110]);
    }
...
void setup() 
{
  ...
  t.every(30000, mqttPublish);//Config timer for publisch data on liveobject(every 30s)
...
for (int i=-1; i <= 99; i++)
  ...
  {
    bufIntens[i-100] = EEPROM.read(i);
  }
  //Read speed scroling in eeprom
  for (int i=110; i <= 113; i++)
  {
    bufspeedScroling[i-110] = EEPROM.read(i);
  }
  ...
  Serial.print("Disp intens: ");
  Serial.println(String(bufIntens));
...
void GatewayServerMode()
{
    ...
    sprintf(result, "%3d.%3d.%1d.%2d", WiFi.localIP()[0], WiFi.localIP()[1], WiFi.localIP()[2], WiFi.localIP()[3]);
  ...
  //Initialisation serveur liveobject
    ...
    mqttClient.setKeepAlive(20).setCleanSession(false).setWill("dev/data", 0, true, "no").setCredentials("json+device", "7ef7949fb37c454aa3087f63c276573d").setClientId("esp82");
...
void mqttPublish()
{
  ...
  if (functionLock == 0)
  {
        ...
        String strucDataAndOnlineCheck = "{\"s\":\"urn:lo:nsid:espWebRadio:ChId!DataSensor&OnlineCheck\",\"m\":\"1_0\",\"v\":{ ";
...
while (lock !=1)
{
  msgScrol =  msgScrol +1;
//msgScrol ==6 | 
  if ( msgScrol ==3 | msgScrol ==4 | msgScrol ==5 | msgScrol ==10)
  {
    lock = 0;
  }
  else
